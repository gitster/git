...
required) and then the body.  For details on how to structure your commit
message (including the recommended **Observation**, **Solution**, and
**Command** flow), see
link:SubmittingPatches.html#meaningful-message[SubmittingPatches].
...
This example commit message follows the formatting guidelines (wrapping
the body to 72 columns, using the imperative mood for the subject, and
explaining the "why" behind the change).  See
link:SubmittingPatches.html#meaningful-message[SubmittingPatches] for a
...
the
link:SubmittingPatches.html#dco[Developer's Certificate of Origin 1.1].
...
haven't noticed your contribution, or they are simply not interested.
...
After a few days, you will hopefully receive a reply to your patchset
with some comments.  Woohoo!  Now you can get back to work.

When responding to reviews, it is important to follow the project's
mailing list etiquette (such as using plaintext, replying inline, and
bottom-posting).  For detailed guidelines, see
link:SubmittingPatches.html#respond-to-reviews[SubmittingPatches].

Be prepared to explain your decisions politely, keep an open mind, and
update your patches to clarify points that reviewers found confusing.
...
When preparing subsequent versions of your patch series, you should
carry forward reviewer trailers (like `Acked-by:` and `Reviewed-by:`)
according to the rules in
link:SubmittingPatches.html#carry-forward-trailers[SubmittingPatches].


[[after-approval]]
=== After Review Approval

Once your patch series has been approved by reviewers, the maintainer
will integrate it into the project's workflow.  Git's codebase uses
several integration branches (`seen`, `next`, `master`, and `maint`)
to manage changes.  For a detailed explanation of how patches flow
through these branches, see the life cycle description in
link:SubmittingPatches.html#patch-flow[SubmittingPatches].

At this stage, you typically do not need to do anything else; the
maintainer will merge your topic branch to `next` and it will
eventually graduate to `master`.
